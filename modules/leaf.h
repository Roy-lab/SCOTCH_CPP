...
class Leaf : public Node {
	public:
		Leaf(int k, double alpha) : Node(k, alpha) {};
		Leaf(gsl_matrix* inputMat, gsl_matrix* initU, gsl_matrix* initV, 
				int k , double alpha, double l, string alias) 
				: Node(initV, k, alpha, alias), lambda(l) {
			isleaf=true;
			X = inputMat;
			U = initU;
			int n = X->size1;
		...
		void set_initial_X(gsl_matrix* initX) {X = initX;};
